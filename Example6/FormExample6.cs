...
namespace Example6
{
    public partial class FormExample6 : Form
    {
        public FormExample6()
        {
            InitializeComponent();
         ...
         // Start Of User Initialization
	        ...
	        gl.MatrixMode(OpenGL.PROJECTION);								// Select The Projection Matrix
	        gl.LoadIdentity();											// Reset The Projection Matrix
	        gl.Perspective(50, (float)Width/(float)Height, 5,  2000); // Set Our Perspective
	        gl.MatrixMode(OpenGL.MODELVIEW);									// Select The Modelview Matrix
	        gl.LoadIdentity();											// Reset The Modelview Matrix

	        gl.Enable(OpenGL.DEPTH_TEST);									// Enable Depth Testing
            ...
            gl.LightModel(OpenGL.LIGHT_MODEL_AMBIENT, lmodel_ambient);		// Set The Ambient Light Model

            gl.LightModel(OpenGL.LIGHT_MODEL_AMBIENT, global_ambient);		// Set The Global Ambient Light Model
            gl.Light(OpenGL.LIGHT0, OpenGL.POSITION, light0pos);				// Set The Lights Position
            gl.Light(OpenGL.LIGHT0, OpenGL.AMBIENT, light0ambient);			// Set The Ambient Light
            gl.Light(OpenGL.LIGHT0, OpenGL.DIFFUSE, light0diffuse);			// Set The Diffuse Light
            gl.Light(OpenGL.LIGHT0, OpenGL.SPECULAR, light0specular);			// Set Up Specular Lighting
            gl.Enable(OpenGL.LIGHTING);										// Enable Lighting
            gl.Enable(OpenGL.LIGHT0);										// Enable Light0

            gl.ShadeModel(OpenGL.SMOOTH);									// Select Smooth Shading

            gl.Material(OpenGL.FRONT, OpenGL.SHININESS, 128);
        ...
        private void openGLControl1_OpenGLDraw(object sender, PaintEventArgs e)
        {
            ...
            gl.ClearColor(0.0f, 0.0f, 0.0f, 0.5f);				// Set The Clear Color To Black
	        gl.Clear (OpenGL.COLOR_BUFFER_BIT | OpenGL.DEPTH_BUFFER_BIT);		// Clear Screen And Depth Buffer
        ...
        protected uint EmptyTexture()
        {
	        ...
	        gl.BindTexture(OpenGL.TEXTURE_2D, txtnumber[0]);			// Bind The Texture
	        gl.TexImage2D(OpenGL.TEXTURE_2D, 0, 4, 128, 128, 0,
		        OpenGL.RGBA, OpenGL.UNSIGNED_BYTE, data);			// Build Texture Using Information In data
        ...
        protected void ProcessHelix()							// Draws A Helix
        {
	        ...
	        gl.Material(OpenGL.FRONT_AND_BACK, OpenGL.AMBIENT_AND_DIFFUSE,glfMaterialColor);
	        gl.Material(OpenGL.FRONT_AND_BACK, OpenGL.SPECULAR,specular);

            r=1.5f;								// Radius

	        gl.Begin(OpenGL.QUADS);						// Begin Drawing Quads
       ...
       protected void ViewOrtho()							// Set Up An Ortho View
        ...
        protected void ViewPerspective()							// Set Up A Perspective View
        {
	        ...
	        gl.MatrixMode( OpenGL.PROJECTION );					// Select Projection
	        gl.PopMatrix();							// Pop The Matrix
	        gl.MatrixMode( OpenGL.MODELVIEW );					// Select Modelview
	        gl.PopMatrix();							// Pop The Matrix
        }
        ...
        public void RenderToTexture()							// Renders To A Texture
        {
	        ...
	        gl.BindTexture(OpenGL.TEXTURE_2D,BlurTexture);			// Bind To The Blur Texture

	        // Copy Our ViewPort To The Blur Texture (From 0,0 To 128,128... No Border)
	        gl.CopyTexImage2D(OpenGL.TEXTURE_2D, 0, OpenGL.LUMINANCE, 0, 0, 128, 128, 0);

	        gl.ClearColor(0.0f, 0.0f, 0.5f, 0.5f);				// Set The Clear Color To Medium Blue
	        gl.Clear(OpenGL.COLOR_BUFFER_BIT | OpenGL.DEPTH_BUFFER_BIT);		// Clear The Screen And Depth Buffer
        ...
        protected void DrawBlur(int times, float inc)					// Draw The Blurred Image
        {
	        ...
	        gl.Disable(OpenGL.TEXTURE_GEN_S);
	        gl.Disable(OpenGL.TEXTURE_GEN_T);

	        gl.Enable(OpenGL.TEXTURE_2D);					// Enable 2D Texture Mapping
	        gl.Disable(OpenGL.DEPTH_TEST);					// Disable Depth Testing
	        gl.BlendFunc(OpenGL.SRC_ALPHA,OpenGL.ONE);				// Set Blending Mode
	        gl.Enable(OpenGL.BLEND);						// Enable Blending
	        gl.BindTexture(OpenGL.TEXTURE_2D,BlurTexture);			// Bind To The Blur Texture
	      ...
	      gl.End();							// Done Drawing Quads
	        ...
	        gl.Enable(OpenGL.DEPTH_TEST);					// Enable Depth Testing
	        gl.Disable(OpenGL.TEXTURE_2D);					// Disable 2D Texture Mapping
	        gl.Disable(OpenGL.BLEND);						// Disable Blending
	        gl.BindTexture(OpenGL.TEXTURE_2D,0);					// Unbind The Blur Texture
    }
